			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				...
				dialog.setPositiveButton("确认", new DialogInterface.OnClickListener() {
				      public void onClick(DialogInterface dialog, int which) {
				    	  ...
				    	  {
				    		  Toast.makeText(activity, "DEBUG乱码", Toast.LENGTH_SHORT).show();
				    	  ...
				    	  else
				    	  {
				    		...
				    		//plug in Database and open it for more operations
				    		  ...
				    		  Toast.makeText(activity, "谢谢", Toast.LENGTH_SHORT).show();			    		  
				    		  dialog.dismiss();
				    		  mDataBase.close();
				    	  }
				         }});
		...
		Text mScoreText = this.createText(mScoreFont,"SCORE: " + mScore,+10);
			...
			if (mScore >= Max)
			{
				if(!isEstablished){
					...
					Text inputText = new Text(0, 0, mTimeFont, "Debug 乱码字串", HorizontalAlign.CENTER);
